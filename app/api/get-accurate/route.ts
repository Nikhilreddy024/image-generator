...
type GetAccurateResult = Record<string, unknown> & {
  success: boolean;
  filename: string;
...
export async function POST(req: Request) {
  try {
    ...
    const result = await runPythonImageScript<GetAccurateResult>(
      "get_accurate_cli.py",
      {
    ...
    const message =
      error instanceof Error ? error.message : "Error during accuracy refinement";
